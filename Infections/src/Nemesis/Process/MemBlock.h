...
public:
    ...
    /// <param name="ptr">Memory address</param>
    /// <param name="own">true if caller will be responsible for block deallocation</param>
    NEMESIS_API MemBlock( class ProcessMemory* mem, ptr_t ptr, bool own = true );
    ...
    /// <param name="prot">Memory protection</param>
    /// <param name="own">true if caller will be responsible for block deallocation</param>
    NEMESIS_API MemBlock( class ProcessMemory* mem, ptr_t ptr, size_t size, DWORD prot, bool own = true, bool physical = false );

    /// <summary>
    /// Move ctor
    /// </summary>
    /// <param name="rhs">Move from</param>
    NEMESIS_API MemBlock( MemBlock&& rhs ) { _pImpl.swap( rhs._pImpl ); }
    NEMESIS_API MemBlock& operator = ( MemBlock&& rhs )
    { 
        // Self assign
    ...
    NEMESIS_API static call_result_t<MemBlock> Allocate(
    ...
    /// <param name="own">false if caller will be responsible for block deallocation</param>
    /// <returns>Memory block. If failed - returned block will be invalid</returns>
    NEMESIS_API static call_result_t<MemBlock> AllocateClosest(
    	class ProcessMemory& process,
        size_t size,
        ptr_t desired,
        DWORD protection = PAGE_EXECUTE_READWRITE,
    ...
    NEMESIS_API call_result_t<ptr_t> Realloc( size_t size, ptr_t desired = 0, DWORD protection = PAGE_EXECUTE_READWRITE );
    ...
    NEMESIS_API NTSTATUS Protect( DWORD protection, uintptr_t offset = 0, size_t size = 0, DWORD* pOld = nullptr );

    /// <summary>
    /// Free memory
    /// </summary>
    /// <param name="size">Size of memory chunk to free. If 0 - whole block is freed</param>
    NEMESIS_API NTSTATUS Free( size_t size = 0 );
    ...
    /// </param>
    /// <returns>Status</returns>
    NEMESIS_API NTSTATUS Read( uintptr_t offset, size_t size, PVOID pResult, bool handleHoles = false );
    ...
    /// <returns>Status</returns>
    NEMESIS_API NTSTATUS Write( uintptr_t offset, size_t size, const void* pData );
    ...
    /// </summary>
    NEMESIS_API void Reset();

    /// <summary>
    /// Memory will not be deallocated upon object destruction
    /// </summary>
    NEMESIS_API inline void Release() { if (_pImpl) _pImpl->_own = false; }
    ...
    NEMESIS_API inline size_t size() const { return _pImpl ? _pImpl->_size : 0; }

    /// <summary>
    /// Get block memory protection
    /// </summary>
    /// <returns>Memory protection flags</returns>
    NEMESIS_API inline DWORD  protection() const { return _pImpl ? _pImpl->_protection : 0; }

    /// <summary>
    /// Validate memory block
    /// <returns>true if memory pointer isn't 0</returns>
    NEMESIS_API inline bool valid() const { return( _pImpl.get() != nullptr && _pImpl->_ptr != 0); }

    /// <summary>
    /// Get memory pointer
    /// </summary>
    /// <returns>Memory pointer</returns>
    NEMESIS_API inline operator ptr_t() const  { return _pImpl ? _pImpl->_ptr : 0; }
