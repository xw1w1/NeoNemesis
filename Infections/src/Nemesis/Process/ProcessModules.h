...
{
    template <>
    struct hash < struct pair<wstring, nemesis::eModType> >
    {
        size_t operator()( const pair<wstring, nemesis::eModType>& value ) const
        {
            hash<wstring> sh;
...
public:
    NEMESIS_API ProcessModules( class Process& proc );
    NEMESIS_API ~ProcessModules();
    ...
    NEMESIS_API ModuleDataPtr GetModule(
    ...
    /// </summary>
    /// <returns>Module data. nullptr if not found</returns>
    NEMESIS_API ModuleDataPtr GetMainModule();
    ...
    NEMESIS_API const mapModules& GetAllModules( eModSeachType search = LdrList );

    /// <summary>
    /// Get list of manually mapped modules
    /// </summary>
    /// <returns>List of modules</returns>
    NEMESIS_API mapModules GetManualModules();
    ...
    NEMESIS_API call_result_t<exportData> GetExport( 
    ...
    /// <param name="name_ord">Function name or ordinal</param>
    /// <returns>Export info. If failed procAddress field is 0</returns>
    NEMESIS_API call_result_t<exportData> GetExport( const std::wstring& modName, const char* name_ord );
    ...
    /// <param name="search">Search type.</param>
    /// <returns>Export info. If failed procAddress field is 0</returns>
    NEMESIS_API call_result_t<exportData> GetNtdllExport(
        const char* name_ord, 
        eModType type = mt_default, 
        eModSeachType search = LdrList 
    );
    ...
    NEMESIS_API call_result_t<ModuleDataPtr> Inject( const std::wstring& path, ThreadPtr pThread = nullptr );
    ...
    /// <returns>true on success</returns>
    NEMESIS_API NTSTATUS Unload( const ModuleDataPtr& hMod );
    ...
    /// <returns>true on success</returns>
    NEMESIS_API bool Unlink( const ModuleDataPtr& mod );
    NEMESIS_API bool Unlink( const ModuleData& mod );
    ...
    NEMESIS_API ModuleDataPtr AddManualModule( const ModuleData& mod );
    ...
    NEMESIS_API ModuleData Canonicalize( const ModuleData& mod, bool manual );
    ...
    NEMESIS_API void RemoveManualModule( const std::wstring& filename, eModType mt );
    ...
    /// <returns>true on success</returns>
    NEMESIS_API bool ValidateModule( module_t base );
