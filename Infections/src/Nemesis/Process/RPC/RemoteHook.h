...
public:
    NEMESIS_API RemoteHook( class ProcessMemory& memory );
    NEMESIS_API ~RemoteHook();
    ...
    NEMESIS_API inline NTSTATUS Apply( eHookType type, uint64_t ptr, fnCallback newFn, ThreadPtr pThread = nullptr )
    {
    ...
    /// <param name="newFn">Callback</param>
    /// <returns>true on success</returns>
    NEMESIS_API inline NTSTATUS AddReturnHook( uint64_t ptr, fnCallback newFn )
    {
    ...
    NEMESIS_API void Remove( uint64_t ptr );
...
private:
    ...
    /// <param name="pThread">Thread to hook. Valid only for HWBP</param>
    /// <returns>true on success</returns>
    NEMESIS_API NTSTATUS ApplyP( eHookType type, uint64_t ptr, fnCallback newFn, const void* pClass = nullptr, ThreadPtr pThread = nullptr );
    ...
    /// <param name="pClass">Class reference.</param>
    /// <returns>true on success</returns>
    NEMESIS_API NTSTATUS AddReturnHookP( uint64_t ptr, fnCallback newFn, const void* pClass = nullptr );
    ...
    /// <param name="ptr">Hooked address</param>
    NEMESIS_API void Restore( const HookData &hook, uint64_t ptr );
