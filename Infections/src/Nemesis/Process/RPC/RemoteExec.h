...
public:
    NEMESIS_API RemoteExec( class Process& proc );
    NEMESIS_API ~RemoteExec();
    ...
    NEMESIS_API NTSTATUS CreateRPCEnvironment( WorkerThreadMode mode = Worker_None, bool bEvent = false );
    ...
    /// <returns>Status</returns>
    NEMESIS_API NTSTATUS ExecInNewThread(
    ...
    /// <returns>Status</returns>
    NEMESIS_API NTSTATUS ExecInWorkerThread( PVOID pCode, size_t size, uint64_t& callResult );
    ...
    /// <returns>Status</returns>
    NEMESIS_API NTSTATUS ExecInAnyThread( PVOID pCode, size_t size, uint64_t& callResult, ThreadPtr& thread );
    ...
    NEMESIS_API DWORD ExecDirect( ptr_t pCode, ptr_t arg );
    ...
    NEMESIS_API NTSTATUS PrepareCallAssembly(
        IAsmHelper& a,
        ptr_t pfn,
        std::vector<nemesis::AsmVariant>& args,
    ...
    NEMESIS_API void AddReturnWithEvent(
        IAsmHelper& a,
    ...
    /// </summary>
    /// <param name="a">Target assembly helper</param>
    NEMESIS_API void SaveCallResult( IAsmHelper& a, uint32_t retOffset = RET_OFFSET )
    {
    ...
    /// </summary>
    /// <returns></returns>
    NEMESIS_API NTSTATUS GetLastStatus()
    {
    ...
    /// </summary>
    NEMESIS_API void TerminateWorker();

    /// <summary>
    /// Get worker thread
    /// </summary>
    /// <returns></returns>
    NEMESIS_API ThreadPtr getWorker() { return _workerThread; }

    /// <summary>
    /// Get execution thread
    /// </summary>
    /// <returns></returns>
    NEMESIS_API ThreadPtr getExecThread() { return _hijackThread ? _hijackThread : _workerThread; }

    /// <summary>
    /// Ge memory routines
    /// </summary>
    /// <returns></returns>
    NEMESIS_API class ProcessMemory& memory() { return _memory; }
