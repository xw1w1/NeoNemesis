...
nemesis::ptr_t RemoteMemory::TranslateAddress( ptr_t address, bool resolveFault /*= true */ )
{
...
void RemoteMemory::HookThread()
{
    ...
    while (_active)
    {
        ...
        {
            NEMESIS_TRACE( L"Allocated 0x%x bytes at %p", opData.allocSize, opData.allocAddress );
            Map( opData.allocAddress, opData.allocSize );
        }
        else
        {
            NEMESIS_TRACE( L"Freed 0x%x bytes at %p", opData.allocSize, opData.allocAddress );
