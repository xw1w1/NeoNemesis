...
{
public:
    NEMESIS_API Thread( DWORD id, class ProcessCore* hProcess, DWORD access = DEFAULT_ACCESS_T );
    NEMESIS_API Thread( HANDLE handle, class ProcessCore* hProcess );
    NEMESIS_API ~Thread();

    NEMESIS_API Thread( const Thread& ) = delete;
    NEMESIS_API Thread& operator =( const Thread& ) = delete;

    NEMESIS_API Thread( Thread&& ) = default;
    NEMESIS_API Thread& operator =( Thread&& ) = default;

    /// <summary>
    /// Get thread ID
    /// </summary>
    /// <returns>Thread ID</returns>
    NEMESIS_API DWORD id() const { return _id; }

    /// <summary>
    /// Get thread handle
    /// </summary>
    /// <returns>Thread hande</returns>
    NEMESIS_API HANDLE handle() const { return _handle; }

    /// <summary>
    /// Check if thread exists
    /// </summary>
    /// <returns>true if thread exists</returns>
    NEMESIS_API bool valid() const { return (_handle && ExitCode() == STILL_ACTIVE); }
    ...
    NEMESIS_API ptr_t teb( _TEB32* pteb ) const;
    ...
    /// <param name="pteb">Process TEB</param>
    /// <returns>TEB pointer</returns>
    NEMESIS_API ptr_t teb( _TEB64* pteb ) const;

    /// <summary>
    /// Get TEB
    /// </summary>
    /// <returns>TEB pointer</returns>
    NEMESIS_API ptr_t teb() const { return teb( (TEB_T*)nullptr ); }

    /// <summary>
    /// Get thread creation time
    /// </summary>
    /// <returns>Thread creation time</returns>
    NEMESIS_API uint64_t startTime() const;

    /// <summary>
    /// Get total execution time(user mode and kernel mode)
    /// </summary>
    /// <returns>Total execution time</returns>
    NEMESIS_API uint64_t execTime() const;

    /// <summary>
    /// Suspend thread
    /// </summary>
    /// <returns>true on success</returns>
    NEMESIS_API bool Suspend();

    /// <summary>
    /// Resumes thread.
    /// </summary>
    /// <returns>true on success</returns>
    NEMESIS_API bool Resume();

    /// <summary>
    /// Check if thread is suspended
    /// </summary>
    /// <returns>true if suspended</returns>
    NEMESIS_API bool Suspended();
    ...
    NEMESIS_API NTSTATUS GetContext( _CONTEXT32& ctx, DWORD flags = CONTEXT_ALL, bool dontSuspend = false );
    ...
    /// <param name="dontSuspend">true if thread shouldn't be suspended before retrieving context</param>
    /// <returns>Status code</returns>
    NEMESIS_API NTSTATUS GetContext( _CONTEXT64& ctx, DWORD flags = CONTEXT64_ALL, bool dontSuspend = false );
    ...
    /// <param name="dontSuspend">true if thread shouldn't be suspended before retrieving context</param>
    /// <returns>Status code</returns>
    NEMESIS_API NTSTATUS SetContext( _CONTEXT32& ctx, bool dontSuspend = false );
    ...
    /// <param name="dontSuspend">true if thread shouldn't be suspended before retrieving context</param>
    /// <returns>Status code</returns>
    NEMESIS_API NTSTATUS SetContext( _CONTEXT64& ctx, bool dontSuspend = false );
    ...
    /// <returns>Status code</returns>
    NEMESIS_API NTSTATUS Terminate( DWORD code = 0 );
    ...
    /// <returns>true on success</returns>
    NEMESIS_API bool Join( int timeout = INFINITE );

    /// <summary>
    /// Get thread exit code
    /// </summary>
    /// <returns>Thread exit code</returns>
    NEMESIS_API DWORD ExitCode() const;
    ...
    NEMESIS_API call_result_t<int> AddHWBP( ptr_t addr, HWBPType type, HWBPLength length );
    ...
    /// <returns>Status code</returns>
    NEMESIS_API NTSTATUS RemoveHWBP( int idx );
    ...
    /// <returns>Status code</returns>
    NEMESIS_API NTSTATUS RemoveHWBP( ptr_t ptr );

    /// <summary>
    /// Close handle
    /// </summary>
    NEMESIS_API void Close();

    NEMESIS_API inline bool operator ==( const Thread& other ) { return (_id == other._id); }
