...
{
public:
    NEMESIS_API static DynImport& Instance()
    {
    ...
    NEMESIS_API FARPROC load( const std::string& name, const std::wstring& modName )
    {
    ...
    /// <returns>true on success</returns>
    NEMESIS_API FARPROC load( const std::string& name, HMODULE hMod )
    {
        CSLock lck( _mapGuard );
