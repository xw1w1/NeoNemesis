...
{ \
    ...
    if(FAILED(hresult)) \
    { \
        NEMESIS_TRACE( "PDB: %s: (%s) failed with HRESULT 0x%08x", __FUNCTION__, #e , hresult ); \
        return hresult; \
    } \
}
...
HRESULT PDBHelper::CoCreateDiaDataSource()
{
    ...
    if (hr == REGDB_E_CLASSNOTREG)
    {
        ...
        if (!hMod)
        {
            ...
            NEMESIS_TRACE( "PDB: Failed to load msdia140.dll, error 0x%08x", err );
        ...
        {
            const auto err = GetLastError();
            NEMESIS_TRACE( "PDB: Failed to get DllGetClassObject from msdia140.dll" );
            return HRESULT_FROM_WIN32( err );
        }
    ...
    if (FAILED( hr ))
    {
        NEMESIS_TRACE( "PDB: %s failed with HRESULT 0x%08x", __FUNCTION__, hr );
    }
