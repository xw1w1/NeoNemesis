...
public:
    NEMESIS_API PEImage( void );
    NEMESIS_API ~PEImage( void );

    NEMESIS_API PEImage( PEImage&& other ) = default;
    ...
    NEMESIS_API NTSTATUS Load( const std::wstring& path, bool skipActx = false );
    ...
    /// <returns>Status code</returns>
    NEMESIS_API NTSTATUS Load( void* pData, size_t size, bool plainData = true );

    /// <summary>
    /// Reload closed image
    /// </summary>
    /// <returns>Status code</returns>
    NEMESIS_API NTSTATUS Reload();

    /// <summary>
    /// Release mapping, if any
    /// </summary>
    /// <param name="temporary">Preserve file paths for file reopening</param>
    NEMESIS_API void Release( bool temporary = false );

    /// <summary>
    /// Parses PE image
    /// </summary>
    /// <returns>Status code</returns>
    NEMESIS_API NTSTATUS Parse( void* pImageBase = nullptr );
    ...
    NEMESIS_API mapImports& GetImports( bool useDelayed = false );

    /// <summary>
    /// Retrieve all exported functions with names
    /// </summary>
    /// <param name="names">Found exports</param>
    NEMESIS_API void GetExports( vecExports& exports );
    ...
    NEMESIS_API int GetTLSCallbacks( module_t targetBase, std::vector<ptr_t>& result ) const;
    ...
    NEMESIS_API uintptr_t DirectoryAddress( int index, AddressType type = VA ) const;
    ...
    NEMESIS_API size_t DirectorySize( int index ) const;
    ...
    NEMESIS_API uintptr_t ResolveRVAToVA( uintptr_t Rva, AddressType type = VA ) const;

    /// <summary>
    /// Get image path
    /// </summary>
    /// <returns>Image path</returns>
    NEMESIS_API inline const std::wstring& path() const { return _imagePath; }

    /// <summary>
    /// Get image name
    /// </summary>
    /// <returns>Image name</returns>
    NEMESIS_API inline std::wstring name() const { return Utils::StripPath( _imagePath ); }

    /// <summary>
    /// Get image load address
    /// </summary>
    /// <returns>Image base</returns>
    NEMESIS_API inline void* base() const { return _pFileBase; }

    /// <summary>
    /// Get image base address
    /// </summary>
    /// <returns>Image base</returns>
    NEMESIS_API inline module_t imageBase() const { return _imgBase; }

    /// <summary>
    /// Get image size in bytes
    /// </summary>
    /// <returns>Image size</returns>
    NEMESIS_API inline uint32_t imageSize() const { return _imgSize; }

    /// <summary>
    /// Get size of image headers
    /// </summary>
    /// <returns>Size of image headers</returns>
    NEMESIS_API inline size_t headersSize() const { return _hdrSize; }
    ...
    NEMESIS_API inline ptr_t entryPoint( module_t base ) const { return ((_epRVA != 0) ? (_epRVA + base) : 0); };

    /// <summary>
    /// Get image sections
    /// </summary>
    /// <returns>Image sections</returns>
    NEMESIS_API inline const vecSections& sections() const { return _sections; }

    /// <summary>
    /// Check if image is an executable file and not a dll
    /// </summary>
    /// <returns>true if image is an *.exe</returns>
    NEMESIS_API inline bool isExe() const { return _isExe; }

    /// <summary>
    /// Check if image is pure IL image
    /// </summary>
    /// <returns>true on success</returns>
    NEMESIS_API inline bool pureIL() const  { return _isPureIL; }
    NEMESIS_API inline int32_t ilFlagOffset() const { return _ILFlagOffset; }

    /// <summary>
    /// Get image type. 32/64 bit
    /// </summary>
    /// <returns>Image type</returns>
    NEMESIS_API inline eModType mType() const { return _is64 ? mt_mod64 : mt_mod32; }

    /// <summary>
    /// Get activation context handle
    /// </summary>
    /// <returns>Actx handle</returns>
    NEMESIS_API inline HANDLE actx() const { return _hctx; }

    /// <summary>
    /// true if image is mapped as plain data file
    /// </summary>
    /// <returns>true if mapped as plain data file, false if mapped as image</returns>
    NEMESIS_API inline bool isPlainData() const { return _isPlainData; }

    /// <summary>
    /// Get manifest resource ID
    /// </summary>
    /// <returns>Manifest resource ID</returns>
    NEMESIS_API inline int manifestID() const { return _manifestIdx; }

    /// <summary>
    /// Get image subsystem
    /// </summary>
    /// <returns>Image subsystem</returns>
    NEMESIS_API inline uint32_t subsystem() const { return _subsystem; }

    /// <summary>
    /// Get manifest resource file
    /// </summary>
    /// <returns>Manifest resource file</returns>
    NEMESIS_API inline const std::wstring& manifestFile() const { return _manifestPath; }

    /// <summary>
    /// If true - no actual PE file available on disk
    /// </summary>
    /// <returns>Flag</returns>
    NEMESIS_API inline bool noPhysFile() const { return _noFile; }

    /// <summary>
    /// DllCharacteristics field of header
    /// </summary>
    /// <returns>DllCharacteristics</returns>
    NEMESIS_API inline uint32_t DllCharacteristics() const { return _DllCharacteristics; }
    ...
    NEMESIS_API ImageNET& net() { return _netImage; }
#endif
