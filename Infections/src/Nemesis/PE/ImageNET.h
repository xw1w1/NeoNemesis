...
public:
    ...
    NEMESIS_API bool Init( const std::wstring& path );
    ...
    /// <returns>true on success</returns>
    NEMESIS_API bool Parse( mapMethodRVA* methods = nullptr );

    /// <summary>
    /// Get image .NET runtime version
    /// </summary>
    /// <returns>runtime version, "n/a" if nothing found</returns>
    NEMESIS_API static std::wstring GetImageRuntimeVer( const wchar_t* ImagePath );
