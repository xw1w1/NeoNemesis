...
protected:
    ...
    NEMESIS_API bool AllocateBuffer( uint8_t* nearest );

    /// <summary>
    /// Temporarily disable hook
    /// </summary>
    /// <returns>true on success</returns>
    NEMESIS_API bool DisableHook();

    /// <summary>
    /// Enable disabled hook
    /// </summary>
    /// <returns>true on success</returns>
    NEMESIS_API bool EnableHook();
    ...
    /// <returns>true on success</returns>
    NEMESIS_API bool ToggleHBP( int index, bool enable );

    /// <summary>
    /// Copy original function bytes
    /// </summary>
    /// <param name="Ptr">Origianl function address</param>
    NEMESIS_API void CopyOldCode( uint8_t* Ptr );
    ...
    NEMESIS_API static LONG NTAPI VectoredHandler ( PEXCEPTION_POINTERS excpt );
    NEMESIS_API static LONG NTAPI Int3Handler     ( PEXCEPTION_POINTERS excpt );
    NEMESIS_API static LONG NTAPI AVHandler       ( PEXCEPTION_POINTERS excpt );
    NEMESIS_API static LONG NTAPI StepHandler     ( PEXCEPTION_POINTERS excpt );
...
protected:
    ...
    NEMESIS_API static std::unordered_map<void*, DetourBase*> _breakpoints;

    // Exception handler
    NEMESIS_API static void* _vecHandler;
};
