...
{
public:
    NEMESIS_API AsmStackAllocator( asmjit::X86Assembler* pAsm, int32_t baseval = 0x28 )
        : _pAsm( pAsm )
    ...
    NEMESIS_API asmjit::Mem AllocVar( int32_t size )
    {
    ...
    NEMESIS_API bool AllocArray( asmjit::Mem arr[], int count, int32_t size )
    {
    ...
    /// </summary>
    /// <returns></returns>
    NEMESIS_API inline intptr_t getTotalSize() const { return disp_ofst; };
