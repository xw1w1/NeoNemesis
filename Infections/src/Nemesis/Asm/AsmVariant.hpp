...
struct AsmVariant
{
    ...
    NEMESIS_API AsmVariant( float _imm_fpu )
        : type( imm_float )
        , size( sizeof( float ) )
        , imm_float_val( _imm_fpu ) { }

    NEMESIS_API AsmVariant( double _imm_fpu )
        : type( imm_double )
        , size( sizeof( double ) )
        , imm_double_val( _imm_fpu ) { }

    NEMESIS_API AsmVariant( asmjit::GpReg _reg )
        : type( reg )
        , size( sizeof( uintptr_t ) )
        , reg_val( _reg ) { }

    // Stack variable
    NEMESIS_API AsmVariant( asmjit::Mem _mem )
        : type( mem )
        , size( sizeof( uintptr_t ) )
        , mem_val( _mem ) { }
    ...
    NEMESIS_API AsmVariant( const asmjit::Mem* _mem )
        : AsmVariant( const_cast<asmjit::Mem*>(_mem) ) { }

    NEMESIS_API AsmVariant( const AsmVariant& ) = default;
    NEMESIS_API AsmVariant( AsmVariant&& ) = default;
    NEMESIS_API AsmVariant& operator =( const AsmVariant& ) = default;

    //
    // Get floating point value as raw data
    //
    NEMESIS_API inline uint32_t getImm_float()  const { return *(reinterpret_cast<const uint32_t*>(&imm_float_val)); }
    NEMESIS_API inline uint64_t getImm_double() const { return *(reinterpret_cast<const uint64_t*>(&imm_double_val)); }

    /// <summary>
    /// Check if argument can be passed in x86 register
    /// </summary>
    /// <returns>true if can</returns>
    NEMESIS_API inline bool reg86Compatible() const
    {
