...
public:
    NEMESIS_API ~NameResolve() = default;

    NEMESIS_API static NameResolve& Instance();

    /// <summary>
    /// Initialize api set map
    /// </summary>
    /// <returns></returns>
    NEMESIS_API bool Initialize();
    ...
    NEMESIS_API NTSTATUS ResolvePath( 
        std::wstring& path,
    ...
    /// <param name="actx">Activation context</param>
    /// <returns></returns>
    NEMESIS_API NTSTATUS ProbeSxSRedirect( std::wstring& path, class Process& proc, HANDLE actx = INVALID_HANDLE_VALUE );
