...
public:
    ...
    NEMESIS_API PatternSearch( const std::vector<uint8_t>& pattern, size_t logAlignment = 0 );
    NEMESIS_API PatternSearch( const std::initializer_list<uint8_t>&& pattern, size_t logAlignment = 0 );
    NEMESIS_API PatternSearch( const std::string& pattern, size_t logAlignment = 0 );
    NEMESIS_API PatternSearch( const char* pattern, size_t len = 0, size_t logAlignment = 0 );
    NEMESIS_API PatternSearch( const uint8_t* pattern, size_t len = 0, size_t logAlignment = 0 );

    NEMESIS_API ~PatternSearch() = default;
    ...
    NEMESIS_API bool SearchWithHandler(
        uint8_t wildcard,
        void* scanStart,
        size_t scanSize,
    ...
    /// <param name="handler">Callback that is called for every match. If it returns true, the search is stopped prematurely.</param>
    /// <returns>true if the callback handler ever returned true (i.e. the search ended prematurely), false otherwise.</returns>
    NEMESIS_API bool SearchRemoteWithHandler(
    ...
    /// <param name="handler">Callback that is called for every match. If it returns true, the search is stopped prematurely.</param>
    /// <returns>true if the callback handler ever returned true (i.e. the search ended prematurely), false otherwise.</returns>
    NEMESIS_API bool SearchRemoteWholeWithHandler(
        class Process& remote,
        bool useWildcard,
        uint8_t wildcard,
        MatchHandler handler
    ...
    /// <param name="maxMatches">Maximum number of matches to collect</param>
    /// <returns>Number of found addresses</returns>
    NEMESIS_API size_t SearchRemote( 
        class Process& remote, 
        uint8_t wildcard, 
        ptr_t scanStart,
        size_t scanSize, 
    ...
    /// <param name="maxMatches">Maximum number of matches to collect</param>
    /// <returns>Number of found addresses</returns>
    NEMESIS_API size_t SearchRemoteWhole( 
        class Process& remote, 
        bool useWildcard, 
        uint8_t wildcard, 
        std::vector<ptr_t>& out,
