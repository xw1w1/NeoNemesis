...
{
public:
    NEMESIS_API MMap( class Process& proc );
    NEMESIS_API ~MMap( void );
    ...
    NEMESIS_API call_result_t<ModuleDataPtr> MapImage(
    ...
    NEMESIS_API NTSTATUS UnmapAllModules();

    /// <summary>
    /// Remove any traces from remote process
    /// </summary>
    /// <returns></returns>
    NEMESIS_API void Cleanup();

    /// <summary>
    /// Reset local data
    /// </summary>
    NEMESIS_API inline void reset() { _images.clear(); _pAContext.Reset(); _usedBlocks.clear(); }
private:
    /// <summary>
    /// Manually map PE image into underlying target process
    /// </summary>
