...
public:
    ...
    NEMESIS_API NTSTATUS Attach( DWORD pid, DWORD access = DEFAULT_ACCESS_P );
    ...
    /// <param name="access">Access mask</param>
    /// <returns>Status code</returns>
    NEMESIS_API NTSTATUS Attach( const wchar_t* name, DWORD access = DEFAULT_ACCESS_P );
    ...
    /// <returns>Status code</returns>
    NEMESIS_API NTSTATUS Attach( HANDLE hProc );
    ...
    /// <returns>Status code</returns>
    NEMESIS_API NTSTATUS CreateAndAttach(
    ...
    /// </summary>
    /// <returns>Status code</returns>
    NEMESIS_API NTSTATUS Detach();

    /// <summary>
    /// Ensure LdrInitializeProcess gets called
    /// </summary>
    /// <returns>Status code</returns>
    NEMESIS_API NTSTATUS EnsureInit();

    /// <summary>
    /// Suspend process
    /// </summary>
    /// <returns>Status code</returns>
    NEMESIS_API NTSTATUS Suspend();

    /// <summary>
    /// Resume process
    /// </summary>
    /// <returns>Status code</returns>
    NEMESIS_API NTSTATUS Resume();

    /// <summary>
    /// Get process ID
    /// </summary>
    /// <returns>Process ID</returns>
    NEMESIS_API inline DWORD pid() const { return _core.pid(); }

    /// <summary>
    /// Checks if process still exists
    /// </summary>
    /// <returns>true if process is valid and exists</returns>
    NEMESIS_API bool valid();
    ...
    NEMESIS_API NTSTATUS Terminate( uint32_t code = 0 );

    /// <summary>
    /// Enumerate all open handles
    /// </summary>
    /// <returns>Found handles or status code</returns>
    NEMESIS_API call_result_t<std::vector<HandleInfo>> EnumHandles();
    ...
    NEMESIS_API static std::vector<DWORD> EnumByName( const std::wstring& name );
    ...
    /// <returns>Status code</returns>
    NEMESIS_API static call_result_t<std::vector<ProcessInfo>> EnumByNameOrPID(
        uint32_t pid,
    ...
    // Subroutines
    //
    NEMESIS_API ProcessCore&     core()       { return _core;       }  // Core routines and native 
    NEMESIS_API ProcessMemory&   memory()     { return _memory;     }  // Memory manipulations
    NEMESIS_API ProcessModules&  modules()    { return _modules;    }  // Module management
    NEMESIS_API ProcessThreads&  threads()    { return _threads;    }  // Threads
    NEMESIS_API RemoteHook&      hooks()      { return _hooks;      }  // Hooking code remotely
    NEMESIS_API RemoteLocalHook& localHooks() { return _localHooks; }  // Hooking code locally
    NEMESIS_API RemoteExec&      remote()     { return _remote;     }  // Remote code execution
    NEMESIS_API MMap&            mmap()       { return _mmap;       }  // Manual module mapping
    NEMESIS_API NtLdr&           nativeLdr()  { return _nativeLdr;  }  // Native loader routines

    // Sugar
    NEMESIS_API const Wow64Barrier& barrier() const { return _core._native->GetWow64Barrier(); }
