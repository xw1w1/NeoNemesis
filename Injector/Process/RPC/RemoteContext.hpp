...
public:
    ...
    // memory object
    NEMESIS_API inline ProcessMemory& memory()
    { 
        return _memory; 
    }
    
    // Native context
    NEMESIS_API inline _CONTEXT64& native()
    { 
        return _ctx; 
    }
    ...
    NEMESIS_API  inline Thread& getThread()
    {
        return _thd;
    }
    ...
    NEMESIS_API inline const ptr_t returnAddress() const
    { 
        ptr_t val = 0;
    ...
    NEMESIS_API inline bool returnAddress( ptr_t val ) const
    { 
    ...
    NEMESIS_API inline void setReturnValue( ptr_t val ) const
    { 
    ...
    /// </summary>
    /// <returns>Return address</returns>
    NEMESIS_API ptr_t unhookReturn()
    {
    ...
    NEMESIS_API DWORD64 getArg( int index )
    {
        if(_x64Target)
        {
            switch (index)
    ...
    /// <returns>true on success</returns>
    NEMESIS_API bool setArg( int index, DWORD64 val )
    {
        if (_x64Target)
        {
            switch (index)
    ...
    /// </summary>
    /// <returns>Last error code, -1 if function failed</returns>
    NEMESIS_API DWORD lastError( DWORD newError )
    {
        ptr_t pteb = 0;
        LONG offset = 0;
    ...
    NEMESIS_API ptr_t getUserContext()
    {
    ...
    /// </summary>
    /// <returns>true on success</returns>
    NEMESIS_API bool setUserContext( ptr_t context )
    {
        auto pteb = _thd.teb( (_TEB64*)nullptr );
        if(pteb)
        {
