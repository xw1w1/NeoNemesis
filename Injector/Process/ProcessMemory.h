...
{
public:
    NEMESIS_API ProcessMemory( class Process* process );
    NEMESIS_API ~ProcessMemory();
    ...
    NEMESIS_API call_result_t<MemBlock> Allocate( size_t size, DWORD protection = PAGE_EXECUTE_READWRITE, ptr_t desired = 0, bool own = true );
    ...
    /// <param name="desired">false if caller will be responsible for block deallocation</param>
    /// <returns>Memory block. If failed - returned block will be invalid</returns>
    NEMESIS_API call_result_t<MemBlock> AllocateClosest( size_t size, DWORD protection = PAGE_EXECUTE_READWRITE, ptr_t desired = 0, bool own = true );
    ...
    NEMESIS_API NTSTATUS Free( ptr_t pAddr, size_t size = 0, DWORD freeType = MEM_RELEASE );
    ...
    /// <returns>Status</returns>
    NEMESIS_API NTSTATUS Query( ptr_t pAddr, PMEMORY_BASIC_INFORMATION64 pInfo );
    ...
    /// <returns>Status</returns>
    NEMESIS_API NTSTATUS Protect( ptr_t pAddr, size_t size, DWORD flProtect, DWORD *pOld = NULL );
    ...
    /// </param>
    /// <returns>Status</returns>
    NEMESIS_API NTSTATUS Read( ptr_t dwAddress, size_t dwSize, PVOID pResult, bool handleHoles = false );
    ...
    /// </param>
    /// <returns>Status</returns>
    NEMESIS_API NTSTATUS Read( const std::vector<ptr_t>& adrList, size_t dwSize, PVOID pResult, bool handleHoles = false );
    ...
    /// <returns>Status</returns>
    NEMESIS_API NTSTATUS Write( ptr_t pAddress, size_t dwSize, const void* pData );
    ...
    /// <param name="pData">Buffer to write</param>
    /// <returns>Status</returns>
    NEMESIS_API NTSTATUS Write( const std::vector<ptr_t>& adrList, size_t dwSize, const void* pData );
    ...
    NEMESIS_API std::vector<MEMORY_BASIC_INFORMATION64> EnumRegions( bool includeFree = false );

    /// <summary>
    /// Get memory protection casting behavior 
    /// </summary>
    /// <returns>current behavior</returns>
    NEMESIS_API MemProtectionCasting protectionCasting() {  return _casting; }

    /// <summary>
    /// Set memory protection casting behavior 
    /// </summary>
    /// <param name="flag">new behavior</param>
    NEMESIS_API void protectionCasting( MemProtectionCasting casting ) { _casting = casting; }

    NEMESIS_API inline class ProcessCore& core() { return _core; }
    NEMESIS_API inline class Process* process()  { return _process; }
